...
def calc_resumen_cartera(cartera_df: pd.DataFrame) -> dict:
    """Devuelve dict con: total_inicial, total_actual, rentabilidad_eur, rentabilidad_pct, por_tipo"""
...
def calc_presupuesto(presupuesto_df: pd.DataFrame) -> dict:
    ...
    gastos = presupuesto_df[["Gastos", "Importe.1"]].dropna(subset=["Gastos"])
...
if pagina == "🏦 Patrimonio":
    ...
    with col1:
        st.metric("Total patrimonio", format_eur(patrimonio_total))
    with col2:
        st.metric("Patrimonio invertido", format_eur(cartera_val))
    with col3:
        st.metric("Liquidez", format_eur(liquidez))
    with col4:
